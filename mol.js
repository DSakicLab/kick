...
////DECLARE ARRAY AND VARIABLES FOR GENERATION OF POINT (P) FOR EACH FRAGMENT
var AllAtomsFragm = [];
AllAtomsFragm[0] = 0;
var RandomAtom = [];
RandomAtom[0] = 0;
...
var temp = 0;
demo += ("New-old frozen fragment"+"<br>");
for (var i=0; i < texts.length; i++) {
if (coorArrF[i] == temp) {
var ada = coorA[i];
var xdx = relX[i];
...
for (var m=1; m <= fragCount; m++) {
demo += ("Random vector V"+m+". Coord: "+fragDx[m]+" "+fragDy[m]+" "+fragDz[m]+" "+"<br>");
ranX[m] = newX[RandomAtom[m]]+fragDx[m];
ranY[m] = newY[RandomAtom[m]]+fragDy[m];
ranZ[m] = newZ[RandomAtom[m]]+fragDz[m];
demo += ("Random point P"+m+". Coord: "+ranX[m]+" "+ranY[m]+" "+ranZ[m]+" "+"<br>");
demo += ("&emsp;Fragment "+m+"<br>");
for (var i=0; i < texts.length; i++) {
if (coorArrF[i] == m) {
var ada = coorA[i];
...
point[i] = new Vector({ x: newX[i]-ranX[m],  y: newY[i]-ranY[m],  z: newZ[i]-ranZ[m],  w: 0 });
pointlength[i]= point[i].length();
...
newX[i]= point[i].x+ranX[m];
newY[i]= point[i].y+ranY[m];
newZ[i]= point[i].z+ranZ[m];
...
//demo1 += ("<br>");
for (var i=0; i < noatoms; i++) {
//atomsAll[i]={elem: AtomsElem[i],x: AtomsX[i], y: AtomsY[i], z: AtomsZ[i], bonds: bond[i], resi: AtomsCount[i]};
